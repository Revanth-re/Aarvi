    ...
    <div style={{ minHeight:"100vh", background:"var(--bg)", display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center", padding:"20px" }}>
      ...
      <div style={{ maxWidth:380, width:"100%", background:"var(--surface)", border:"1px solid var(--border)", borderRadius:20, padding:"36px 28px", boxShadow:"var(--shadow-lg)" }}>
        ...
        <button onClick={loginWithGoogle} disabled={redirecting}
          style={{ width:"100%", display:"flex", alignItems:"center", justifyContent:"center", gap:12, padding:"14px 20px", borderRadius:12, border:"1.5px solid var(--border2)", background:"var(--surface2)", cursor: redirecting ? "default" : "pointer", fontSize:15, fontWeight:600, color:"var(--text)", transition:"all .15s", marginBottom:16, opacity: redirecting ? .7 : 1 }}
          onMouseEnter={e => { if (!redirecting) { (e.currentTarget as HTMLButtonElement).style.borderColor = "var(--accent)"; (e.currentTarget as HTMLButtonElement).style.background = "var(--bg2)"; } }}
